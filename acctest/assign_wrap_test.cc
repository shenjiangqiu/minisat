...
TEST_CASE("EventQueue") {
  ...
  SECTION("True test for event_queue") {
    ...
    w1->add_clause_addr(14, 5);
    EventQueue m_queue;
    auto evalue = EventValue(EventType::FinishAndSendClause, 0, 10, w1,
                             HardwareType::ClauseUnit, 1);
    auto event = Event(evalue, 0, 10);
    m_queue.push(event);
    REQUIRE(m_queue.get_next_event().size == 10);
    auto evalue2 = EventValue(EventType::FinishAndSendClause, 11, 10, w1,
                              HardwareType::ClauseUnit, 1);
...
TEST_CASE("real acc test") {
  ...
  SECTION("simple test") {
    ...
    w1->add_clause_literals_addr(10, 12345);
    w1->add_clause_literals_addr(10, 12346);
    w1->add_clause_literals_addr(10, 22347);

    w1->add_clause_addr(11, 2 << 9);
    w1->add_clause_literals_addr(11, 112345);
    w1->add_clause_literals_addr(11, 112346);
    w1->add_clause_literals_addr(11, 122347);
    w1->add_clause_literals_addr(11, 312345);
    w1->add_clause_literals_addr(11, 312346);
    w1->add_clause_literals_addr(11, 322347);
    auto w2 = af.create(2, 100, 10, w1, 1);
    w2->add_clause_addr(12, 3 << 9);
    w2->add_clause_literals_addr(12, 412345);
    w2->add_clause_literals_addr(12, 412346);
    w2->add_clause_literals_addr(12, 422347);
    auto w3 = af.create(3, 100, 12, w2, 3);
    // std::shared_ptr<...> create_acc<...>(int watcher_proc_size, int
    // watcher_proc_num, int clause_proc_num, int miss_latency, int
    // watcher_process_latency, int clause_process_latency)
  ...
  SECTION("parralle test") {
    // todo generate multiple assigments at the same time, those assignment
    // should be able to run in parallel
  }
  SECTION("limited hardware test") {
    // todo now we have more task and limited hardware resource, the test should
    // be able to queued.
  }
}